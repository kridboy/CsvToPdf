...
public class CsvToPdfConverter {
    ...
    public void convertFiles() throws IOException {
        for (Map.Entry<String, Table> entrySet : createCsvTableMap().entrySet()) {
            try (PdfWriter writer = new PdfWriter("pdf/" + entrySet.getKey() + ".pdf");
                 PdfDocument pdfDocument = new PdfDocument(writer);
                 Document document = new Document(pdfDocument)) {
                document.add(entrySet.getValue());
            }
        }
    }

    private Map<String, Table> createCsvTableMap() throws IOException {
